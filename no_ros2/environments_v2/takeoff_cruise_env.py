...
class MockPylonRacingEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if terminated:
            ...
            # 1. Altitude Score
            alt_err = abs(z - TARGET_ALT)
            if alt_err < ALT_MARGIN:
                reward += 0.2
            elif alt_err > ALT_MARGIN * 2:
                reward -= 0.2
            ...
            # 3. Forward Speed Score
            v = obs[9]
            if v > 6.0:
                reward += 0.1
                
            # 4. Alive Bonus during takeoff roll
    ...
    def reset(self, seed=None, options=None):
        ...
        # Start on the ground with enough forward speed for physics stability
        _V = 8.0
        self._state = np.zeros(12, dtype=np.float64)
        self._state[0] = 0.0                      # x
        self._state[1] = 0.0                      # y
        self._state[2] = 0.0                      # z (ground)
        self._state[3] = _V                       # u (forward speed)
        self._state[5] = 0.0                      # w 
        self._state[7] = 0.0                      # theta 
