...
CHORD_M           = 0.20                             # average chord (m)

# Aerodynamic coefficients (from fixedwing_4ch.py p_defaults)
CL0               = 0.20    # lift at zero AoA
CL_ALPHA          = 5.2     # 1/rad
CL_ELEVATOR       = 0.40    # lift increment per unit elevator deflection (added)
...
CM_ELEVATOR       = 0.90    # pitch from elevator (positive = nose up)
CY_BETA           = -0.35   # side-force due to sideslip
CL_P              = -0.5    # roll damping
CN_R              = -0.12   # yaw damping
CL_AILERON        = 0.10    # roll from aileron
CN_RUDDER         = 0.12    # yaw from rudder
...
# Structural limits
G_LIMIT_POS       = 6.0                # max positive load factor
G_LIMIT_NEG       = -3.0               # max negative load factor

# Angular rate limits  rad/s
P_MAX             = np.deg2rad(360)    # roll rate
...
class FlightDynamics:
    ...
    def enforce_actuator_limits(
        ...
        # --- aileron [0] ---
        max_d_ail = AILERON_RATE_MAX * dt / AILERON_MAX_RAD
    ...
    def compute_aero_forces(
        ...
        FIX: elevator now contributes via CL_ELEVATOR (not CM_ELEVATOR),
             keeping moment and force coefficients properly separated.
        """
        alpha = np.clip(alpha, ALPHA_MIN_RAD, ALPHA_MAX_RAD)
        qbar  = q_bar(v)
        S     = WING_AREA_M2

        # Lift — base + AoA + elevator camber effect
    ...
    def compute_moments(
        ...
        FIX: method was indented incorrectly (outside the class), and alpha
             is now clamped to the valid AoA envelope before use.
        """
        alpha  = np.clip(alpha, ALPHA_MIN_RAD, ALPHA_MAX_RAD)
        qbar   = q_bar(v)
        S      = WING_AREA_M2
        b      = WINGSPAN_M
        c      = CHORD_M
        v_safe = max(v, 1.0)   # avoid division by zero at very low speeds

        # Roll moment — roll damping + aileron authority
        roll_moment = qbar * S * b * (
            CL_P * (p_rate * b / (2 * v_safe))
            + CL_AILERON * aileron * AILERON_MAX_RAD
        )

        # Pitch moment — static stability + pitch damping + elevator
        ...
        # Yaw moment — yaw damping + rudder authority
        yaw_moment = qbar * S * b * (
...
class EnvelopeReward:
    ...
    def stall_penalty(stall_fraction: float) -> float:
        ...
        excess = max(stall_fraction - 0.85, 0.0)
        return -8.0 * excess ** 2
