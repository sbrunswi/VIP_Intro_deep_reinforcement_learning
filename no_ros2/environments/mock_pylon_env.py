...
class MockPylonRacingEnv(gym.Env):
    ...
    def __init__(self, dt=0.1, seed=None, course=None):
        super().__init__()
        self.course = course  # stored but unused in mock physics
    ...
    def _integrate(self, action):
        ...
        v_after = (
            np.sqrt(self._state[3] ** 2 + self._state[4] ** 2 + self._state[5] ** 2)
            + 1e-9
        )
    ...
    def step(self, action):
        ...
        terminated = bool((self.has_taken_off and z < 0.1) or (z < -0.5))
    ...
    def reset(self, seed=None, options=None):
        ...
        self._state = np.zeros(6, dtype=np.float32)
        self._state[2] = 2.0   # start at 2 m altitude so "level" actions hold altitude
